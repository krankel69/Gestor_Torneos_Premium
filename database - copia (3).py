# database.py
# Cerebro de datos: todas las consultas SQLite del gestor de torneos.
# Esquema unificado compatible con cualquier .db creado por esta app.
import sqlite3
...
class GestorBaseDeDatos:
    def __init__(self, db_path):
        ...
        # --- DETECCIÓN DINÁMICA DE ESQUEMA (LIGA VS MUNDIAL) ---
        self.cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='selecciones'")
        es_mundial = self.cursor.fetchone() is not None

        if es_mundial:
            self.tabla_equipos = "selecciones"
            self.col_equipo_id = "seleccion_id"
        else:
            self.tabla_equipos = "equipos"
            self.col_equipo_id = "equipo_id"

        self.crear_tablas()

    def crear_tablas(self):
        # Solo crea la estructura de la Liga si no estamos en un Mundial
        if self.tabla_equipos == "equipos":
            self.cursor.execute("""
        ...
        self.cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS jugadores (
                ...
                {self.col_equipo_id} INTEGER,
                goles_marcados INTEGER DEFAULT 0,
                FOREIGN KEY ({self.col_equipo_id}) REFERENCES {self.tabla_equipos} (id) ON DELETE CASCADE
            )
        """)
        self.cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS partidos (
                ...
                FOREIGN KEY (equipo_local_id) REFERENCES {self.tabla_equipos} (id) ON DELETE CASCADE,
                FOREIGN KEY (equipo_visitante_id) REFERENCES {self.tabla_equipos} (id) ON DELETE CASCADE
            )
        """)
        self.cursor.execute("""
    ...
    def _migrar_columnas(self):
        """Compatibilidad con BDs antiguas (nodriza.db, etc)."""
        migraciones = [
            ("jugadores", "goles_marcados", "INTEGER DEFAULT 0"),
            ("partidos", "jugado", "INTEGER DEFAULT 0"),
            ("partidos", "archivado", "INTEGER DEFAULT 0"),
            (self.tabla_equipos, "ciudad", "TEXT DEFAULT ''"),
        ]
        for tabla, columna, definicion in migraciones:
            try:
                ...
                pass
        # Sincronizar jugado con convencion nodriza.db (goles_local=-1 = pendiente)
        self.cursor.execute("UPDATE partidos SET jugado=1 WHERE goles_local!=-1 AND goles_visitante!=-1 AND jugado=0")
        self.conn.commit()
    ...
    def obtener_id_equipo(self, nombre):
        self.cursor.execute(f"SELECT id FROM {self.tabla_equipos} WHERE nombre = ?", (nombre,))
    ...
    def obtener_nombres_equipos(self):
        self.cursor.execute(f"SELECT nombre FROM {self.tabla_equipos} ORDER BY nombre")
        return [row["nombre"] for row in self.cursor.fetchall()]

    def obtener_equipos(self):
        self.cursor.execute(f"SELECT * FROM {self.tabla_equipos} ORDER BY nombre")
    ...
    def obtener_jugadores_completo(self):
        query = f"""
            ...
            FROM jugadores j 
            LEFT JOIN {self.tabla_equipos} e ON j.{self.col_equipo_id} = e.id 
            ORDER BY j.nombre
        """
    ...
    def obtener_jugadores_por_equipo(self, nombre_equipo):
        ...
        if equipo_id:
            self.cursor.execute(f"SELECT * FROM jugadores WHERE {self.col_equipo_id} = ? ORDER BY nombre", (equipo_id,))
            return self.cursor.fetchall()
        return []
    ...
    def obtener_partidos_con_equipos(self, fecha_filtro=None, mostrar_archivados=0):
        base_query = f"""
            ...
            FROM partidos p
            JOIN {self.tabla_equipos} el ON p.equipo_local_id = el.id
            JOIN {self.tabla_equipos} ev ON p.equipo_visitante_id = ev.id
            WHERE p.archivado = ?
        """
    ...
    def obtener_goleadores_top(self):
        query = f"""
            ...
            FROM jugadores j 
            JOIN {self.tabla_equipos} e ON j.{self.col_equipo_id} = e.id 
    ...
    def recalcular_y_obtener_clasificacion(self):
        self.cursor.execute(f"UPDATE {self.tabla_equipos} SET pj=0, pg=0, pe=0, pp=0, gf=0, gc=0, pts=0")
        self.cursor.execute("SELECT * FROM partidos WHERE jugado = 1")
        partidos = self.cursor.fetchall()
        for p in partidos:
            gl, gv = p["goles_local"], p["goles_visitante"]
            self.cursor.execute(f"UPDATE {self.tabla_equipos} SET gf = gf + ?, gc = gc + ?, pj = pj + 1 WHERE id = ?", (gl, gv, p["equipo_local_id"]))
            self.cursor.execute(f"UPDATE {self.tabla_equipos} SET gf = gf + ?, gc = gc + ?, pj = pj + 1 WHERE id = ?", (gv, gl, p["equipo_visitante_id"]))
            if gl > gv:
                self.cursor.execute(f"UPDATE {self.tabla_equipos} SET pg = pg + 1, pts = pts + 3 WHERE id = ?", (p["equipo_local_id"],))
                self.cursor.execute(f"UPDATE {self.tabla_equipos} SET pp = pp + 1 WHERE id = ?", (p["equipo_visitante_id"],))
            elif gv > gl:
                self.cursor.execute(f"UPDATE {self.tabla_equipos} SET pp = pp + 1 WHERE id = ?", (p["equipo_local_id"],))
                self.cursor.execute(f"UPDATE {self.tabla_equipos} SET pg = pg + 1, pts = pts + 3 WHERE id = ?", (p["equipo_visitante_id"],))
            else:
                self.cursor.execute(f"UPDATE {self.tabla_equipos} SET pe = pe + 1, pts = pts + 1 WHERE id = ?", (p["equipo_local_id"],))
                self.cursor.execute(f"UPDATE {self.tabla_equipos} SET pe = pe + 1, pts = pts + 1 WHERE id = ?", (p["equipo_visitante_id"],))
        self.conn.commit()
    ...
    def obtener_clasificacion_actual(self):
        ...
        # 1. Extraemos la clasificación básica de la base de datos
        self.cursor.execute(f"SELECT id, nombre, pj, pg, pe, pp, gf, gc, pts FROM {self.tabla_equipos} ORDER BY pts DESC, (gf - gc) DESC, gf DESC")
        equipos_filas = self.cursor.fetchall()

        # Convertimos las filas a diccionarios para poder procesarlos
        equipos = [dict(row) for row in equipos_filas]

        # 2. Agrupamos a los equipos por la cantidad de puntos que tienen
        ...
        # 3. Procesamos los grupos de mayor a menor puntuación
        for pts in sorted(grupos_puntos.keys(), reverse=True):
            grupo = grupos_puntos[pts]

            if len(grupo) == 1:
                # Si no hay empate, el equipo pasa directo a la tabla
                clasificacion_final.append(grupo[0])
            else:
                # --- HAY EMPATE: CALCULAMOS EL GOLAVERAJE PARTICULAR (MINI-LIGA) ---
                ids_empatados = tuple(eq["id"] for eq in grupo)

                # Preparamos los marcadores a 0 solo entre ellos
                mini_liga = {eq["id"]: {"pts_mini": 0, "dif_mini": 0, "equipo_original": eq} for eq in grupo}

                # Buscamos en las actas SOLO los partidos jugados entre los equipos empatados
                ...
                # Sumamos los puntos y goles de sus enfrentamientos directos
                ...
                # Ordenamos a los empatados usando la regla estricta de La Liga
                grupo_ordenado = sorted(
                    mini_liga.values(),
                    key=lambda x: (
                        x["pts_mini"],  # 1º Puntos en los cara a cara
                        x["dif_mini"],  # 2º Dif. de goles en los cara a cara
                        (x["equipo_original"]["gf"] - x["equipo_original"]["gc"]),  # 3º Dif. de goles en toda la liga
                        x["equipo_original"]["gf"],  # 4º Goles a favor en toda la liga
                    ),
                    reverse=True,
                )

                # Añadimos a los equipos ya desempatados a la lista final
    ...
    def actualizar_clasificacion_manual(self, equipo_nombre, pj, pg, pe, pp, gf, gc, pts):
        ...
        if equipo_id:
            self.cursor.execute(f"UPDATE {self.tabla_equipos} SET pj=?, pg=?, pe=?, pp=?, gf=?, gc=?, pts=? WHERE id=?", (pj, pg, pe, pp, gf, gc, pts, equipo_id))
            self.conn.commit()

    def agregar_equipo(self, nombre, ciudad):
        try:
            if self.tabla_equipos == "selecciones":
                # Protegemos el insert para el esquema del Mundial
                self.cursor.execute(f"INSERT INTO {self.tabla_equipos} (nombre, grupo) VALUES (?, ?)", (nombre, "A"))
            else:
                self.cursor.execute(f"INSERT INTO {self.tabla_equipos} (nombre, ciudad) VALUES (?, ?)", (nombre, ciudad))
            self.conn.commit()
            return True
    ...
    def eliminar_equipo_por_nombre(self, nombre):
        self.cursor.execute(f"DELETE FROM {self.tabla_equipos} WHERE nombre = ?", (nombre,))
        self.conn.commit()

    def agregar_jugador(self, nombre_jugador, nombre_equipo):
        equipo_id = self.obtener_id_equipo(nombre_equipo)
        if equipo_id:
            self.cursor.execute(f"INSERT INTO jugadores (nombre, {self.col_equipo_id}) VALUES (?, ?)", (nombre_jugador, equipo_id))
            self.conn.commit()
    ...
    def recalcular_goleadores_totales(self):
        ...
        self.cursor.execute("SELECT jugador_id, SUM(goles) as total FROM goleadores_partido GROUP BY jugador_id")
        goles_realizados = self.cursor.fetchall()
        for g in goles_realizados:
            self.cursor.execute("UPDATE jugadores SET goles_marcados = ? WHERE id = ?", (g["total"], g["jugador_id"]))
        self.conn.commit()
    ...
    def establecer_goles_totales_manual(self, nombre_jugador, nuevo_total):
        jugador_id = self.obtener_id_jugador(nombre_jugador)
        if jugador_id:
            self.cursor.execute("UPDATE jugadores SET goles_marcados = ? WHERE id = ?", (nuevo_total, jugador_id))
            self.conn.commit()
            return True
        return False
