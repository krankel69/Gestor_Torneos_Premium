...
class GestorBaseDeDatos:
    ...
    _COLS_EQUIPOS = {
        ...
        "pj": ("pj",), "pg": ("pg", "wins"), "pe": ("pe", "draws"),
        "pp": ("pp", "losses"), "gf": ("gf", "goals_for"),
        "gc": ("gc", "goals_against"), "pts": ("pts", "points"),
    }
    _COLS_JUGADORES = {
        "id":             ("id",),
        "nombre":         ("nombre", "name"),
    ...
    def _detectar_todo(self):
        ...
        # Determinar tipo de torneo automaticamente
        nombre_db = os.path.basename(self.db_name).lower()
        tiene_fase     = self._c_par.get("fase") is not None
        tiene_selec    = self._t_equipos == "selecciones"
        es_mundial_nom = any(p in nombre_db for p in ("mundial","world","euro","copa","champions","ucl"))

        if tiene_selec or (tiene_fase and es_mundial_nom):
            self.tipo_torneo = "mundial"
        elif tiene_fase or es_mundial_nom:
            self.tipo_torneo = "mundial"
        else:
    ...
    def _migrar_columnas(self):
        ...
        try:
            ...
            self.cursor.execute(
                f"UPDATE {self._t_partidos} SET jugado=1 WHERE {gl}!=-1 AND {gv}!=-1 AND jugado=0"
            )
            self.conn.commit()
        except Exception:
            pass
    ...
    def agregar_equipo(self, nombre, ciudad=""):
        """Agrega un equipo a la BD con conexión nueva."""
        try:
            # Crear conexión nueva (no usar global bloqueada)
            import sqlite3
            ...
            conn.execute(
                f"INSERT INTO {self._t_equipos} ({cn},{cc}) VALUES (?,?)", 
                (nombre, ciudad)
            )
            conn.commit()
            conn.close()
            ...
            print(f"Error agregando equipo: {e}")
            return False
    ...
    def obtener_jugadores_completo(self):
        t_j = self._t_jugadores; t_e = self._t_equipos
        m_j = self._c_jug;       m_e = self._c_eq
    ...
    def obtener_jugadores_por_equipo(self, nombre_equipo):
        ...
        if eid:
            ...
            self.cursor.execute(
                f"SELECT * FROM {self._t_jugadores} WHERE {col}=? ORDER BY nombre", (eid,)
            )
            return self.cursor.fetchall()
        return []

    def agregar_jugador(self, nombre_jugador, nombre_equipo):
        """Agrega un jugador con conexión nueva."""
        try:
            eid = self.obtener_id_equipo(nombre_equipo)
            if not eid:
                return False
            ...
            conn.execute(
                f"INSERT INTO {self._t_jugadores} ({cn},{ce}) VALUES (?,?)", 
                (nombre_jugador, eid)
            )
            conn.commit()
            conn.close()
    ...
    def recalcular_goleadores_totales(self):
        ...
        self.cursor.execute(
            f"SELECT jugador_id, SUM(goles) AS total FROM {self._t_goleadores} GROUP BY jugador_id"
        )
        for g in self.cursor.fetchall():
            self.cursor.execute(
                f"UPDATE {self._t_jugadores} SET {cg}=? WHERE id=?", (g["total"], g["jugador_id"])
            )
        self.conn.commit()
    ...
    def establecer_goles_totales_manual(self, nombre_jugador, nuevo_total):
        ...
        if jid:
            cg = self._c_jug.get("goles_marcados") or "goles_marcados"
            self.cursor.execute(
                f"UPDATE {self._t_jugadores} SET {cg}=? WHERE id=?", (nuevo_total, jid)
            )
            self.conn.commit()
            return True
        return False
    ...
    def _q_partidos_base(self):
        m = self._c_par; m_e = self._c_eq
        t = self._t_partidos; t_e = self._t_equipos
        gl  = m.get("goles_local")         or "goles_local"
        gv  = m.get("goles_visitante")     or "goles_visitante"
    ...
    def obtener_jornadas(self, mostrar_archivados=0):
        ...
        try:
            self.cursor.execute(
                f"SELECT DISTINCT {cf} AS fecha FROM {self._t_partidos} "
                f"WHERE COALESCE({ca},0)=? ORDER BY {cf} DESC", (mostrar_archivados,)
            )
    ...
    def agregar_partido(self, local, visitante, fecha):
        ...
        if id_l and id_v:
            ...
            cf = m.get("fecha")               or "fecha"
            self.cursor.execute(
                f"INSERT INTO {self._t_partidos} ({cl},{cv},{cf},jugado) VALUES (?,?,?,0)",
                (id_l, id_v, fecha)
            )
            self.conn.commit()
            return True
    ...
    def registrar_resultado_partido(self, partido_id, goles_local, goles_visitante):
        ...
        gv = m.get("goles_visitante") or "goles_visitante"
        self.cursor.execute(
            f"UPDATE {self._t_partidos} SET {gl}=?,{gv}=?,jugado=1 WHERE id=?",
    ...
    def toggle_archivo_partido(self, partido_id, estado):
        self.cursor.execute(
            f"UPDATE {self._t_partidos} SET archivado=? WHERE id=?", (estado, partido_id)
        )
        self.conn.commit()
    ...
    def obtener_goleadores_por_partido(self, partido_id):
        t_g = self._t_goleadores; t_j = self._t_jugadores
        cn = self._c_jug.get("nombre") or "nombre"
        self.cursor.execute(f"""
    ...
    def insertar_goleador(self, partido_id, nombre_jugador, cantidad_goles):
        ...
        if jid:
            self.cursor.execute(
                f"INSERT INTO {self._t_goleadores} (partido_id,jugador_id,goles) VALUES (?,?,?)",
    ...
    def limpiar_goleadores_de_partido(self, partido_id):
        self.cursor.execute(
            f"DELETE FROM {self._t_goleadores} WHERE partido_id=?", (partido_id,)
        )
        self.conn.commit()
    ...
    def recalcular_y_obtener_clasificacion(self):
        m_e = self._c_eq; m_p = self._c_par
        t_e = self._t_equipos; t_p = self._t_partidos
        pj = m_e.get('pj','pj'); pg = m_e.get('pg','pg'); pe = m_e.get('pe','pe')
        pp = m_e.get('pp','pp'); gf = m_e.get('gf','gf'); gc = m_e.get('gc','gc')
        pts = m_e.get('pts','pts')
        ...
        self.cursor.execute(
            f"UPDATE {t_e} SET {pj}=0,{pg}=0,{pe}=0,{pp}=0,{gf}=0,{gc}=0,{pts}=0"
        )
        ...
        for p in self.cursor.fetchall():
            ...
            id_l, id_v = p[cl], p[cv]
            self.cursor.execute(
                f"UPDATE {t_e} SET {gf}={gf}+?,{gc}={gc}+?,{pj}={pj}+1 WHERE id=?", (g_l,g_v,id_l)
            )
            self.cursor.execute(
                f"UPDATE {t_e} SET {gf}={gf}+?,{gc}={gc}+?,{pj}={pj}+1 WHERE id=?", (g_v,g_l,id_v)
            )
            if g_l > g_v:
    ...
    def obtener_clasificacion_actual(self):
        m = self._c_eq
        pts_c = m.get("pts","pts"); gf_c = m.get("gf","gf"); gc_c = m.get("gc","gc")
        self.cursor.execute(f"""
        ...
        for pts in sorted(grupos.keys(), reverse=True):
            ...
            for p in self.cursor.fetchall():
                ...
                if g_l > g_v:   mini[il]["pts_mini"] += 3
                elif g_v > g_l: mini[iv]["pts_mini"] += 3
                else:           mini[il]["pts_mini"] += 1; mini[iv]["pts_mini"] += 1
            for item in sorted(mini.values(),
                key=lambda x: (x["pts_mini"],x["dif_mini"],
                               x["orig"]["gf"]-x["orig"]["gc"],x["orig"]["gf"]),
                reverse=True):
