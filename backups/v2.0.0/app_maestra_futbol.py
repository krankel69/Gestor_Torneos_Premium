...
class AppMaestra(tk.Frame):
    ...
    def _crear_ui(self):
        ...
        t_panel = tb.Frame(nb); nb.add(t_panel, text="📊 Panel de Control")
    ...
    def _ui_panel_control(self, parent):
        ...
        tb.Label(frm_prog_content, text=f"{total_partidos}/{max_partidos} partidos jugados ({porcentaje}%)", 
                 font="-size 11 -weight bold").pack(anchor=W, pady=(0,10))
        
        pb = tb.Progressbar(frm_prog_content, value=porcentaje, maximum=100, bootstyle="success", length=500)
        pb.pack(fill=X, pady=10)
        
        tb.Label(frm_prog_content, text=f"Progreso: {porcentaje}%", 
                 font="-size 10", foreground="#666").pack(anchor=W)
        ...
        if proximo:
            tb.Label(frm_prox_content, text=f"📅 {proximo['fecha']}", 
                     font="-size 12 -weight bold", foreground="#0066cc").pack(anchor=W, pady=(0,10))
            tb.Label(frm_prox_content, text=f"⚽ {proximo['local']} vs {proximo['visitante']}", 
                     font="-size 11").pack(anchor=W)
        else:
            tb.Label(frm_prox_content, text="✓ Todos los partidos han sido jugados", 
                     font="-size 11", foreground="#00aa00").pack(anchor=W)
        ...
        tb.Label(frm_lider, text="👑 LÍDER", font="-size 10 -weight bold", 
                 foreground="#ff6600").pack(anchor=W, pady=(0,5))
        if lider:
            tb.Label(frm_lider, text=lider["nombre"], font="-size 12 -weight bold").pack(anchor=W)
            tb.Label(frm_lider, text=f"📊 {lider['pts']} pts | ⚽ {lider['gf']}:{lider['gc']} | +{lider['dif']}", 
                     font="-size 9", foreground="#666").pack(anchor=W, pady=(5,0))
        else:
        ...
        tb.Label(frm_gol, text="🥅 MÁXIMO GOLEADOR", font="-size 10 -weight bold", 
                 foreground="#00aa00").pack(anchor=W, pady=(0,5))
        if maximo_gol:
            tb.Label(frm_gol, text=maximo_gol["jugador"], font="-size 12 -weight bold").pack(anchor=W)
            tb.Label(frm_gol, text=f"⚽ {maximo_gol['goles']} goles | {maximo_gol['equipo']}", 
                     font="-size 9", foreground="#666").pack(anchor=W, pady=(5,0))
        else:
    ...
    def _ui_clasificacion(self, parent):
        ...
        headers = {"pos":"#","nombre":"Equipo","pj":"PJ","pg":"PG","pe":"PE",
                   "pp":"PP","gf":"GF","gc":"GC","dif":"DIF","pts":"PTS",
                   "racha":"Racha","rend_l":"Rend.L","rend_v":"Rend.V"}
        anchos  = {"pos":35,"nombre":160,"pj":40,"pg":40,"pe":40,
                   "pp":40,"gf":40,"gc":40,"dif":50,"pts":50,
                   "racha":50,"rend_l":60,"rend_v":60}
        for c in cols_c:
    ...
    def _ui_partidos(self, parent):
        ...
        tb.Button(frm_ctrl, text="🖨 Generar Acta PDF", bootstyle="info-outline",
                  command=self._on_generar_acta_pdf).pack(fill=X, pady=3)
    ...
    def _ui_grupos(self, parent):
        ...
        tb.Button(frm_der, text="🖨 Generar Acta PDF", bootstyle="warning-outline",
                  command=self._on_generar_acta_pdf_mundial).grid(row=12, column=0, columnspan=3, sticky="ew", pady=3)
        # Goleadores globales
    ...
    def _ui_liguilla(self, parent):
        ...
        tb.Button(frm_pdf, text="🖨 Generar Acta", bootstyle="warning-outline",
                  command=self._on_generar_acta_champions).pack(side=LEFT, padx=5, pady=5)
    ...
    def _obtener_racha(self, nombre_equipo):
        ...
        try:
            conn = _conn(self.db_path)
            eq_id = conn.execute(
                "SELECT id FROM equipos WHERE nombre=?", (nombre_equipo,)
            ).fetchone()
    ...
    def _generar_acta_pdf(self, partido_id):
        ...
        try:
            ...
            # Tabla de goleadores
            data_goleadores = [
                [f"<b>{partido['local']}</b>", "Goles", f"<b>{partido['visitante']}</b>", "Goles"]
            ]
...
class DashboardPrincipal(tk.Frame):
    ...
    def __init__(self, root, db_path, callback_torneo_seleccionado):
        ...
        tb.Label(frm_nuevo, text="Crea un torneo personalizado con tu propia BD", 
                 font="-size 10", foreground="#666").pack(pady=(10,10), padx=10)
        tb.Button(frm_nuevo, text="🆕 Crear Torneo Personalizado", 
                  bootstyle="warning", width=30,
                  command=self._crear_nuevo_torneo).pack(padx=10, pady=10)
        # Footer
        ...
        def on_crear(resultado):
            if resultado:
                ...
                try:
                    ...
                    conn.execute("PRAGMA journal_mode = WAL")
                    conn.executescript(SCHEMA)
                    conn.execute("INSERT INTO torneo_info (nombre, tipo, temporada) VALUES (?, ?, ?)",
                                (nombre, tipo_torneo, ""))
                    conn.commit()
                    conn.close()
