...
from ui_components_v2 import StatWidget, Badge
...
def _normalizar_nombre(s):
    ...
    _EXTRAS = str.maketrans("øØæÆłŁðþ", "oOaAlLdt")
...
CARPETA_BBDD_OVERRIDE = r"D:\Manu\Documentos\FUTBOL\Gestor_Torneos_Premium\MAESTRO"
...
def _carpeta_bbdd():
    if CARPETA_BBDD_OVERRIDE:
        return CARPETA_BBDD_OVERRIDE
    ruta = os.path.join(os.path.dirname(os.path.abspath(__file__)), "MAESTRO")
    os.makedirs(ruta, exist_ok=True)
    return ruta


FASES_ELIMINATORIAS = {
    "mundial": ["Fase de grupos", "Dieciseisavos de final", "Octavos de final", "Cuartos de final", "Semifinal", "Tercer Puesto", "Final"],
    "euro": ["Fase de grupos", "Octavos de final", "Cuartos de final", "Semifinal", "Tercer Puesto", "Final"],
    "copa": ["Fase de liga", "Playoffs", "Octavos de final", "Cuartos de final", "Semifinal", "Final"],
    "liga": [],
}
...
def crear_bd_nueva(ruta, nombre, tipo, temporada=""):
    ...
    conn.execute("INSERT OR IGNORE INTO torneo_info (nombre,tipo,temporada) VALUES (?,?,?)", (nombre, tipo, temporada))
    conn.commit()
    conn.close()
...
class VentanaGoleadores(tk.Toplevel):
    ...
    def _build(self):
        ...
        tb.Button(f, text="🗑 Limpiar todos los goleadores", bootstyle="danger-outline", command=self._limpiar).pack(fill=X, pady=5)
    ...
    def _cargar(self):
        ...
        if not rows:
            self.lbl_vacio = tb.Label(self.tree.master, text="Este partido no tiene goleadores registrados.", foreground="#aaaaaa", font="-size 9")
...
class AppMaestra(tk.Frame):
    ...
    def __init__(self, root, db_path, torneo_info):
        ...
        try:
            ...
            if "tarjetas_amarillas_local" not in p_cols:
                for col in ["tarjetas_amarillas_local", "tarjetas_amarillas_visitante", "tarjetas_rojas_local", "tarjetas_rojas_visitante"]:
    ...
    def _crear_ui(self):
        ...
        tb.Button(barra, text="🔄 Refrescar Todo", bootstyle="secondary-outline", command=self._refresh_all).pack(side=RIGHT, padx=5)
        tb.Button(barra, text="🏆 Cambiar Torneo", bootstyle="info-outline", command=self._cambiar_torneo).pack(side=RIGHT, padx=5)
        tb.Button(barra, text="✖ Salir", bootstyle="danger-outline", command=self._al_cerrar).pack(side=RIGHT, padx=5)

        nb = tb.Notebook(self)
        nb.pack(fill=BOTH, expand=True, padx=10, pady=(0, 10))

        # PANEL DE CONTROL (en todos los torneos)
        t_panel = tb.Frame(nb)
        nb.add(t_panel, text="📊 Panel de Control")
        self._ui_panel_control(t_panel)

        if self.tipo == "liga":
            t_cls = tb.Frame(nb)
            nb.add(t_cls, text="📊 Clasificación")
            t_jug = tb.Frame(nb)
            nb.add(t_jug, text="🏃 Jugadores")
            t_par = tb.Frame(nb)
            nb.add(t_par, text="⚽ Partidos")
            t_gol = tb.Frame(nb)
            nb.add(t_gol, text="🥅 Goleadores")
            t_exp = tb.Frame(nb)
            nb.add(t_exp, text="📄 Exportar")
            ...
            t_lig = tb.Frame(nb)
            nb.add(t_lig, text="📋 Liguilla")
            t_pla = tb.Frame(nb)
            nb.add(t_pla, text="🔥 Playoff")
            t_eli = tb.Frame(nb)
            nb.add(t_eli, text="🏆 Eliminatorias")
            t_gol = tb.Frame(nb)
            nb.add(t_gol, text="🥅 Goleadores")
            t_equ = tb.Frame(nb)
            nb.add(t_equ, text="👥 Equipos y Jugadores")
            t_exp = tb.Frame(nb)
            nb.add(t_exp, text="📄 Exportar")
    ...
    def _ui_panel_control(self, parent):
        """Panel de control con resumen del torneo: progreso, próxima jornada, destacados."""
        frm = tb.Frame(parent, padding=20)
        frm.pack(fill=BOTH, expand=True)

        # ══════════════════════════════════════════════════════
        # Sección 1: Progreso del Torneo (MEJORADO)
        # ══════════════════════════════════════════════════════
        frm_progreso = tb.LabelFrame(frm, text=" 📊 PROGRESO DEL TORNEO ")
        frm_progreso.pack(fill=X, padx=10, pady=(0, 20))

        frm_prog_content = tb.Frame(frm_progreso, padding=15)
        frm_prog_content.pack(fill=X)

        try:
            conn = _conn(self.db_path)
            total_partidos = conn.execute("SELECT COUNT(*) FROM partidos WHERE jugado=1").fetchone()[0]
            max_partidos = conn.execute("SELECT COUNT(*) FROM partidos").fetchone()[0]
        ...
        # Row de números bonitos
        frm_stats = tb.Frame(frm_prog_content)
        frm_stats.pack(fill=X, pady=10)

        stat1 = StatWidget(frm_stats, label="Partidos Jugados", value=total_partidos, color=Colors.PRIMARY)
        stat1.pack(side="left", padx=5, expand=True, fill="both")

        stat2 = StatWidget(frm_stats, label="Equipos", value=total_equipos, color=Colors.SUCCESS)
        stat2.pack(side="left", padx=5, expand=True, fill="both")

        # Barra de progreso
        porcentaje = int((total_partidos / max_partidos * 100)) if max_partidos > 0 else 0
        pb = tb.Progressbar(frm_prog_content, value=porcentaje, maximum=100, bootstyle="success", length=500)
        pb.pack(fill=X, pady=10)

        tb.Label(frm_prog_content, text=f"Progreso: {porcentaje}%", font="-size 10", foreground="#666").pack(anchor=W)

        # ══════════════════════════════════════════════════════
        # Sección 2: Próxima Jornada / Próximo Partido
        # ══════════════════════════════════════════════════════
        frm_proxima = tb.LabelFrame(frm, text=" 📅 PRÓXIMO PARTIDO ")
        frm_proxima.pack(fill=X, padx=10, pady=(0, 20))

        frm_prox_content = tb.Frame(frm_proxima, padding=15)
        frm_prox_content.pack(fill=X)
        ...
        if proximo:
            tb.Label(frm_prox_content, text=f"📅 {proximo['fecha']}", font="-size 12 -weight bold", foreground="#0066cc").pack(anchor=W, pady=(0, 10))
            tb.Label(frm_prox_content, text=f"⚽ {proximo['local']} vs {proximo['visitante']}", font="-size 11").pack(anchor=W)
        else:
            tb.Label(frm_prox_content, text="✓ Todos los partidos han sido jugados", font="-size 11", foreground="#00aa00").pack(anchor=W)

        # ══════════════════════════════════════════════════════
        # Sección 3: Destacados (Líder + Máximo Goleador)
        # ══════════════════════════════════════════════════════
        frm_destacados = tb.LabelFrame(frm, text=" 🏆 DESTACADOS ")
        frm_destacados.pack(fill=X, padx=10)

        frm_dest_content = tb.Frame(frm_destacados, padding=15)
        frm_dest_content.pack(fill=X)

        # Líder
        conn = _conn(self.db_path)
        lider = conn.execute("""
            SELECT nombre, pts, gf, gc, (gf-gc) as dif
            FROM equipos
        ...
        # Máximo goleador
        maximo_gol = conn.execute("""
            SELECT j.nombre as jugador, eq.nombre as equipo, COUNT(*) as goles
            FROM goleadores_partido gp
            JOIN jugadores j ON gp.jugador_id = j.id
        ...
        # Layout de destacados en dos columnas
        frm_cols = tb.Frame(frm_dest_content)
        frm_cols.pack(fill=X)

        # Columna 1: Líder
        frm_lider = tb.Frame(frm_cols)
        frm_lider.pack(side=LEFT, fill=BOTH, expand=True, padx=(0, 10))

        tb.Label(frm_lider, text="👑 LÍDER", font="-size 10 -weight bold", foreground="#ff6600").pack(anchor=W, pady=(0, 5))
        if lider:
            tb.Label(frm_lider, text=lider["nombre"], font="-size 12 -weight bold").pack(anchor=W)
            tb.Label(frm_lider, text=f"📊 {lider['pts']} pts | ⚽ {lider['gf']}:{lider['gc']} | +{lider['dif']}", font="-size 9", foreground="#666").pack(anchor=W, pady=(5, 0))
        else:
            tb.Label(frm_lider, text="Sin datos", font="-size 9", foreground="#999").pack(anchor=W)

        # Columna 2: Máximo goleador
        frm_gol = tb.Frame(frm_cols)
        frm_gol.pack(side=RIGHT, fill=BOTH, expand=True, padx=(10, 0))

        tb.Label(frm_gol, text="🥅 MÁXIMO GOLEADOR", font="-size 10 -weight bold", foreground="#00aa00").pack(anchor=W, pady=(0, 5))
        if maximo_gol:
            tb.Label(frm_gol, text=maximo_gol["jugador"], font="-size 12 -weight bold").pack(anchor=W)
            tb.Label(frm_gol, text=f"⚽ {maximo_gol['goles']} goles | {maximo_gol['equipo']}", font="-size 9", foreground="#666").pack(anchor=W, pady=(5, 0))
        else:
    ...
    def _ui_clasificacion(self, parent):
        ...
        self.entry_eq_nombre = tb.Entry(frm_izq)
        self.entry_eq_nombre.pack(fill=X)
        tb.Label(frm_izq, text="Ciudad:").pack(anchor=W, pady=(8, 0))
        self.entry_eq_ciudad = tb.Entry(frm_izq)
        self.entry_eq_ciudad.pack(fill=X)
        tb.Button(frm_izq, text="➕ Añadir Equipo", bootstyle=SUCCESS, command=self._on_aniadir_equipo).pack(fill=X, pady=(10, 3))
        tb.Button(frm_izq, text="🗑 Eliminar Equipo Seleccionado", bootstyle="danger-outline", command=self._on_eliminar_equipo).pack(fill=X, pady=3)
        tb.Button(frm_izq, text="📁 Importar Equipos (CSV)", bootstyle=INFO, command=self._on_cargar_equipos_csv).pack(fill=X, pady=3)

        tk.Frame(frm_izq, height=1, bg="#cccccc").pack(fill=X, pady=15)
        tb.Button(frm_izq, text="🔢 Recalcular Clasificación", bootstyle="warning", command=self._on_recalcular_clasificacion).pack(fill=X, pady=3)
        tb.Button(frm_izq, text="✏️ Editar Estadísticas Manual", bootstyle="warning-outline", command=self._on_guardar_clasificacion_manual).pack(fill=X, pady=3)
        tb.Button(frm_izq, text="🔄 Nueva Temporada", bootstyle="secondary-outline", command=self._on_nueva_temporada).pack(fill=X, pady=3)

        tk.Frame(frm_izq, height=1, bg="#cccccc").pack(fill=X, pady=15)
        tb.Label(frm_izq, text="Historial de Simulacros:").pack(anchor=W)
        tb.Button(frm_izq, text="📋 Ver Historial Simulacros", bootstyle="info-outline", command=self._on_simulacro_historial).pack(fill=X, pady=3)

        # Panel derecho: tabla clasificación
        tb.Label(frm_der, text="Tabla de Posiciones", font="-size 13 -weight bold").pack(pady=5, anchor=W)
        cols_c = ("pos", "nombre", "pj", "pg", "pe", "pp", "gf", "gc", "dif", "pts", "racha", "rend_l", "rend_v")
        self.tree_clas = tb.Treeview(frm_der, columns=cols_c, show="headings", height=22, bootstyle="primary")
        headers = {"pos": "#", "nombre": "Equipo", "pj": "PJ", "pg": "PG", "pe": "PE", "pp": "PP", "gf": "GF", "gc": "GC", "dif": "DIF", "pts": "PTS", "racha": "Racha", "rend_l": "Rend.L", "rend_v": "Rend.V"}
        anchos = {"pos": 35, "nombre": 160, "pj": 40, "pg": 40, "pe": 40, "pp": 40, "gf": 40, "gc": 40, "dif": 50, "pts": 50, "racha": 50, "rend_l": 60, "rend_v": 60}
        for c in cols_c:
    ...
    def _ui_jugadores(self, parent):
        ...
        self.entry_jug_nombre = tb.Entry(frm_ctrl)
        self.entry_jug_nombre.pack(fill=X)
        ...
        tb.Button(frm_ctrl, text="➕ Añadir Jugador", bootstyle=SUCCESS, command=self._on_aniadir_jugador).pack(fill=X, pady=(10, 3))
        tb.Button(frm_ctrl, text="🗑 Eliminar Jugador", bootstyle="danger-outline", command=self._on_eliminar_jugador).pack(fill=X, pady=3)
        tb.Button(frm_ctrl, text="📁 Importar Jugadores (carpeta CSV)", bootstyle=INFO, command=self._on_cargar_jugadores_carpeta).pack(fill=X, pady=3)
        tb.Button(frm_ctrl, text="🧹 Limpiar Duplicados", bootstyle="warning-outline", command=self._on_limpiar_duplicados_jugadores).pack(fill=X, pady=3)
        tb.Button(frm_ctrl, text="⚠️ Borrar TODOS los Jugadores", bootstyle="danger", command=self._on_borrar_todos_jugadores).pack(fill=X, pady=3)

        tk.Frame(frm_ctrl, height=1, bg="#cccccc").pack(fill=X, pady=15)
        tb.Label(frm_ctrl, text="Editar goles totales (doble-clic en tabla):", font="-size 9", foreground="#888").pack(anchor=W)
        ...
        self.tree_jugadores.column("id", width=50, anchor="center")
        self.tree_jugadores.heading("nombre", text="Jugador")
        self.tree_jugadores.column("nombre", width=200)
        self.tree_jugadores.heading("equipo", text="Equipo")
        self.tree_jugadores.column("equipo", width=160)
        self.tree_jugadores.heading("goles", text="Goles")
        self.tree_jugadores.column("goles", width=60, anchor="center")
    ...
    def _ui_partidos(self, parent):
        ...
        tb.Checkbutton(frm_top, text="Ver Archivo Histórico", variable=self._mostrar_archivados, command=self._on_toggle_historico).pack(side=LEFT, padx=10)
        ...
        tb.Button(frm_ctrl, text="📅 Programar Partido", bootstyle=SUCCESS, command=self._on_crear_partido).pack(fill=X, pady=(8, 3))
        tb.Button(frm_ctrl, text="📁 Importar Jornada (CSV)", bootstyle=INFO, command=self._on_cargar_jornada_csv).pack(fill=X, pady=3)
        ...
        self.entry_gl = tb.Entry(frm_goles, width=5)
        self.entry_gl.pack(side=LEFT, padx=2)
        tb.Label(frm_goles, text="V:").pack(side=LEFT)
        self.entry_gv = tb.Entry(frm_goles, width=5)
        self.entry_gv.pack(side=LEFT, padx=2)
        tb.Button(frm_ctrl, text="✅ Registrar Resultado", bootstyle=SUCCESS, command=self._on_registrar_resultado).pack(fill=X, pady=(5, 3))
        tb.Button(frm_ctrl, text="✏️ Editar Partido / Goles", bootstyle="warning-outline", command=self._abrir_editar_partido).pack(fill=X, pady=3)
        tb.Button(frm_ctrl, text="🖨 Generar Acta PDF", bootstyle="info-outline", command=self._on_generar_acta_pdf).pack(fill=X, pady=3)

        tk.Frame(frm_ctrl, height=1, bg="#cccccc").pack(fill=X, pady=10)
        tb.Button(frm_ctrl, text="📦 Archivar Partido", bootstyle="secondary-outline", command=self._on_toggle_archivo_partido).pack(fill=X, pady=3)
        tb.Button(frm_ctrl, text="🗑 Eliminar Partido", bootstyle="danger-outline", command=self._on_eliminar_partido).pack(fill=X, pady=3)

        # Panel tabla
        tb.Label(frm_tbl, text="Partidos (doble-clic → ver goleadores)", font="-size 11").pack(pady=2, anchor=W)
        cols_p = ("id", "fecha", "local", "visitante", "res", "jugado")
        self.tree_partidos = tb.Treeview(frm_tbl, columns=cols_p, show="headings", height=12)
        self.tree_partidos.heading("id", text="ID")
        self.tree_partidos.column("id", width=40, anchor="center")
        self.tree_partidos.heading("fecha", text="Fecha")
        self.tree_partidos.column("fecha", width=90, anchor="center")
        self.tree_partidos.heading("local", text="Local")
        self.tree_partidos.column("local", width=150)
        self.tree_partidos.heading("visitante", text="Visitante")
        self.tree_partidos.column("visitante", width=150)
        self.tree_partidos.heading("res", text="Res.")
        self.tree_partidos.column("res", width=60, anchor="center")
        self.tree_partidos.heading("jugado", text="Estado")
        self.tree_partidos.column("jugado", width=70, anchor="center")
        ...
        self.lbl_sin_goles = tb.Label(frm_tbl, text="(sin goleadores registrados para este partido)", foreground="#aaaaaa", font="-size 9")
        ...
        self.spin_gol_cantidad.pack(side=LEFT)
        self.spin_gol_cantidad.set(1)
        tb.Button(frm_ag1, text="⚽ Añadir", bootstyle="info", command=self._on_aniadir_goleador_partido).pack(side=LEFT, padx=8)
        tb.Button(frm_ag1, text="🗑 Limpiar goleadores", bootstyle="danger-outline", command=self._on_limpiar_goleadores_partido).pack(side=LEFT)
    ...
    def _ui_goleadores_tab(self, parent):
        ...
        tb.Button(frm_ctrl, text="🔄 Recalcular desde partidos", bootstyle="warning", command=self._on_recalcular_goleadores).pack(fill=X, pady=5)
        tb.Button(frm_ctrl, text="📁 Importar goleadores (CSV)", bootstyle=INFO, command=self._on_importar_goleadores_csv).pack(fill=X, pady=5)
        ...
        self.tree_goleadores.heading("pos", text="#")
        self.tree_goleadores.column("pos", width=40, anchor="center")
        self.tree_goleadores.heading("jugador", text="Jugador")
        self.tree_goleadores.column("jugador", width=200)
        self.tree_goleadores.heading("equipo", text="Equipo")
        self.tree_goleadores.column("equipo", width=160)
        self.tree_goleadores.heading("goles", text="Goles")
        self.tree_goleadores.column("goles", width=60, anchor="center")
    ...
    def _ui_grupos(self, parent):
        ...
        for grp in grupos:
            ...
            tb.Button(frm_botones, text=grp, width=6, command=_on_grupo).pack(side=LEFT, padx=2, pady=2)
        ...
        # Tabla de partidos
        tb.Label(frm_izq, text="📅 Partidos del Grupo", font="-size 11 -weight bold", bootstyle="inverse-primary").pack(fill=X, pady=(0, 5))
        ...
        # Tabla de posiciones
        tb.Label(frm_izq, text="🏆 Posiciones", font="-size 11 -weight bold", bootstyle="inverse-success").pack(fill=X, pady=(10, 5))
        ...
        self.entry_mundial_gl = tb.Entry(frm_der, width=8)
        self.entry_mundial_gl.grid(row=2, column=1, sticky=W)
        tb.Label(frm_der, text="Goles Visit.:", font="-weight bold").grid(row=3, column=0, sticky=W, pady=3)
        self.entry_mundial_gv = tb.Entry(frm_der, width=8)
        self.entry_mundial_gv.grid(row=3, column=1, sticky=W)
        ...
        frm_gol.grid(row=13, column=0, columnspan=3, sticky="ew", pady=(10, 0))
        self.tree_gol_mundial = tb.Treeview(frm_gol, columns=("jugador", "goles"), show="headings", height=8, bootstyle="success")
    ...
    def _ui_liguilla(self, parent):
        ...
        tb.Label(frm_top, text="⚽ LIGUILLA CHAMPIONS - 36 EQUIPOS", font="-size 13 -weight bold", bootstyle="inverse-info").pack(pady=5)
        tb.Label(frm_top, text="Clasificación única | 1-8: Octavos directo | 9-24: Playoff | 25-36: Eliminados", font="-size 10", foreground="#666").pack(pady=3)

        # Tabla de clasificación
        cols = ("pos", "equipo", "pj", "pg", "pe", "pp", "gf", "gc", "dif", "pts")
        self.tree_liguilla = tb.Treeview(parent, columns=cols, show="headings", height=20)
        self.tree_liguilla.heading("pos", text="Pos")
        self.tree_liguilla.column("pos", width=35, anchor="c")
        self.tree_liguilla.heading("equipo", text="Equipo")
        self.tree_liguilla.column("equipo", width=200)
        ...
        # Panel de generador de PDF
        frm_pdf = tb.LabelFrame(parent, text=" 🖨 Generar Actas ")
        frm_pdf.pack(fill=X, padx=10, pady=(0, 10))

        tb.Label(frm_pdf, text="Selecciona partido:", font="-weight bold").pack(side=LEFT, padx=5, pady=5)
        self.combo_partido_champions = tb.Combobox(frm_pdf, state="readonly", width=50)
        self.combo_partido_champions.pack(side=LEFT, padx=5, pady=5, fill=X, expand=True)

        tb.Button(frm_pdf, text="🖨 Generar Acta", bootstyle="warning-outline", command=self._on_generar_acta_champions).pack(side=LEFT, padx=5, pady=5)

        # ════════════════════════════════════════════════════════════
        # TABLA DE PARTIDOS
        # ════════════════════════════════════════════════════════════

        frm_part = tb.LabelFrame(parent, text=" 📅 PARTIDOS DE LA LIGUILLA ")
        frm_part.pack(fill=BOTH, expand=True, padx=10, pady=(10, 0))

        # Selector de jornada
        frm_jorn = tb.Frame(frm_part)
        frm_jorn.pack(fill=X, padx=10, pady=10)
        tb.Label(frm_jorn, text="Jornada:", font="-weight bold").pack(side=LEFT, padx=5)
        self.combo_jornada_champions = tb.Combobox(frm_jorn, values=["L1", "L2", "L3", "L4", "L5", "L6", "L7", "L8", "P", "E"], state="readonly", width=10)
        self.combo_jornada_champions.pack(side=LEFT, padx=5)
        self.combo_jornada_champions.bind("<<ComboboxSelected>>", lambda e: self._cargar_partidos_champions())

        # Tabla de partidos
        cols_p = ("fecha", "local", "visitante", "res")
        self.tree_partidos_champions = tb.Treeview(frm_part, columns=cols_p, show="headings", height=10)
        self.tree_partidos_champions.heading("fecha", text="Fecha")
        self.tree_partidos_champions.column("fecha", width=100)
        self.tree_partidos_champions.heading("local", text="Local")
        self.tree_partidos_champions.column("local", width=150)
        self.tree_partidos_champions.heading("visitante", text="Visitante")
        self.tree_partidos_champions.column("visitante", width=150)
        self.tree_partidos_champions.heading("res", text="Resultado")
        self.tree_partidos_champions.column("res", width=80)

        sb_p = tb.Scrollbar(frm_part, orient=VERTICAL, command=self.tree_partidos_champions.yview)
        self.tree_partidos_champions.configure(yscrollcommand=sb_p.set)
        self.tree_partidos_champions.pack(side=LEFT, fill=BOTH, expand=True, padx=10, pady=10)
        sb_p.pack(side=RIGHT, fill=Y)

        # ════════════════════════════════════════════════════════════
        # PANEL DERECHO - RESULTADOS Y BOTONES
        # ════════════════════════════════════════════════════════════

        frm_der = tb.LabelFrame(frm_part, text=" 🎯 REGISTRAR RESULTADO ")
        frm_der.pack(fill=Y, side=RIGHT, padx=(10, 0), pady=10)

        tb.Label(frm_der, text="Partido:", font="-weight bold").grid(row=0, column=0, columnspan=3, sticky="w", pady=5)
        self.combo_partido_champ = tb.Combobox(frm_der, state="readonly", width=30)
        self.combo_partido_champ.grid(row=1, column=0, columnspan=3, sticky="ew", pady=5)

        tb.Label(frm_der, text="Goles Local:").grid(row=2, column=0, sticky="w", pady=5)
        self.spin_goles_local_champ = tb.Spinbox(frm_der, from_=-1, to=10, width=5)
        self.spin_goles_local_champ.set(-1)
        self.spin_goles_local_champ.grid(row=2, column=1, sticky="w", padx=5)

        tb.Label(frm_der, text="Goles Visitante:").grid(row=3, column=0, sticky="w", pady=5)
        self.spin_goles_visit_champ = tb.Spinbox(frm_der, from_=-1, to=10, width=5)
        self.spin_goles_visit_champ.set(-1)
        self.spin_goles_visit_champ.grid(row=3, column=1, sticky="w", padx=5)

        tb.Button(frm_der, text="✅ Grabar Marcador", bootstyle=SUCCESS, command=self._grabar_resultado_champions).grid(row=4, column=0, columnspan=3, sticky="ew", pady=5)
        tb.Button(frm_der, text="⚽ Añadir Goleador", bootstyle="info", command=self._grabar_gol_champions).grid(row=5, column=0, columnspan=3, sticky="ew", pady=3)
        tb.Button(frm_der, text="🗑 Limpiar Resultado", bootstyle="danger-outline", command=self._limpiar_resultado_champions).grid(row=6, column=0, columnspan=3, sticky="ew", pady=3)

        self._cargar_liguilla()

    def _cargar_partidos_champions(self):
        """Carga partidos de Champions desde CSV según jornada seleccionada."""
        if not hasattr(self, "tree_partidos_champions"):
            return

        jornada = self.combo_jornada_champions.get()
        if not jornada:
            return

        # Ruta del CSV
        csv_path = f"D:\\Manu\\Documentos\\FUTBOL\\Gestor_Torneos_Premium\\MAESTRO\\CHAMPIONS\\PARTIDOS\\{jornada}.csv"

        # Cargar partidos
        partidos = self._cargar_partidos_csv(csv_path)

        # Limpiar tabla
        self.tree_partidos_champions.delete(*self.tree_partidos_champions.get_children())

        # Mostrar partidos
        for p in partidos:
            self.tree_partidos_champions.insert("", "end", values=(p["fecha"], p["local"], p["visitante"], "–"))

    def _grabar_resultado_champions(self):
        """Guarda resultado en Champions."""
        txt = self.combo_partido_champ.get()
        if not txt:
            messagebox.showwarning("Aviso", "Selecciona un partido.")
            return
        try:
            pid = int(txt.split(" - ")[0])
            gl = int(self.spin_goles_local_champ.get())
            gv = int(self.spin_goles_visit_champ.get())
        except:
            messagebox.showerror("Error", "Datos inválidos.")
            return

        conn = _conn(self.db_path)
        conn.execute("UPDATE partidos SET goles_local=?, goles_visitante=?, jugado=1 WHERE id=?", (gl, gv, pid))
        conn.commit()
        conn.close()
        messagebox.showinfo("Éxito", "Resultado guardado.")
        self._cargar_partidos_champions()

    def _grabar_gol_champions(self):
        """Abre ventana para agregar goleadores en Champions."""
        txt = self.combo_partido_champ.get()
        if not txt:
            messagebox.showwarning("Aviso", "Selecciona un partido.")
            return
    ...
    def _limpiar_resultado_champions(self):
        """Borra resultado y goleadores en Champions."""
        txt = self.combo_partido_champ.get()
        if not txt:
            messagebox.showwarning("Aviso", "Selecciona un partido.")
            return
        ...
        if not messagebox.askyesno("Confirmar", "¿Eliminar resultado y goleadores?"):
            return
    ...
    def _cargar_liguilla(self):
        ...
        if not hasattr(self, "tree_liguilla"):
            return
        ...
        # Cargar partidos en combo
        partidos = conn.execute("""
            SELECT p.id, el.nombre as local, ev.nombre as visitante, p.goles_local, p.goles_visitante
            FROM partidos p
            JOIN equipos el ON p.equipo_local_id = el.id
            JOIN equipos ev ON p.equipo_visitante_id = ev.id
        ...
        if hasattr(self, "combo_partido_champions"):
            self.combo_partido_champions["values"] = opciones
        if hasattr(self, "combo_partido_champ"):
            self.combo_partido_champ["values"] = opciones
        ...
        for pos, r in enumerate(rows, 1):
            ...
            self.tree_liguilla.insert("", "end", values=(pos, r["nombre"], r["pj"], r["pg"], r["pe"], r["pp"], r["gf"], r["gc"], dif, r["pts"]), tags=tag)
    ...
    def _ui_playoff(self, parent):
        ...
        tb.Label(frm_top, text="🔥 PLAYOFF CHAMPIONS - IDA Y VUELTA", font="-size 13 -weight bold", bootstyle="inverse-warning").pack(pady=5)
        tb.Label(frm_top, text="16 equipos (posiciones 9-24) → 8 ganadores a Octavos", font="-size 10", foreground="#666").pack(pady=3)

        frm_btn = tb.Frame(frm_top)
        frm_btn.pack(fill=X, pady=10)
        tb.Button(frm_btn, text="🎲 Generar Playoff Automático", bootstyle="warning", command=self._generar_playoff).pack(side=LEFT, padx=5)
        tb.Button(frm_btn, text="🔄 Cargar Playoff", bootstyle="info-outline", command=self._cargar_playoff).pack(side=LEFT, padx=5)
        ...
        self.tree_playoff.heading("tipo", text="Cruce")
        self.tree_playoff.column("tipo", width=60, anchor="c")
        self.tree_playoff.heading("local", text="Local")
        self.tree_playoff.column("local", width=120)
        self.tree_playoff.heading("visitante", text="Visitante")
        self.tree_playoff.column("visitante", width=120)
        self.tree_playoff.heading("ida", text="IDA")
        self.tree_playoff.column("ida", width=50, anchor="c")
        self.tree_playoff.heading("vuelta", text="VUELTA")
        self.tree_playoff.column("vuelta", width=50, anchor="c")
        self.tree_playoff.heading("agregado", text="Agregado")
        self.tree_playoff.column("agregado", width=60, anchor="c")
        self.tree_playoff.heading("ganador", text="Ganador")
        self.tree_playoff.column("ganador", width=100)
    ...
    def _cargar_playoff(self):
        ...
        if not hasattr(self, "tree_playoff"):
            return
    ...
    def _ui_eliminatorias_champions(self, parent):
        ...
        tb.Label(frm_top, text="🏆 ELIMINATORIAS CHAMPIONS - IDA Y VUELTA", font="-size 13 -weight bold", bootstyle="inverse-success").pack(pady=5)
        ...
        for fase in ["Octavos", "Cuartos", "Semifinal", "Final"]:
            tb.Button(frm_sel, text=fase, width=15, command=lambda f=fase: self._cargar_fase_eliminatoria(f)).pack(side=LEFT, padx=2)
    ...
    def _cargar_fase_eliminatoria(self, fase):
        ...
        if not hasattr(self, "tree_eliminatorias"):
            return
    ...
    def _ui_eliminatorias(self, parent):
        ...
        tb.Label(frm_izq, text=lbl_top, font="-size 11 -weight bold", bootstyle="inverse-info").pack(fill=X, pady=(0, 5))
        self.tree_clasificados = tb.Treeview(frm_izq, columns=("grupo", "pos", "pais", "pts"), show="headings", height=10, bootstyle="info")
        ...
        tb.Button(frm_izq, text="🔄 Refrescar Clasificados", bootstyle="info-outline", command=self._cargar_eliminatorias).pack(fill=X, pady=6)

        # Goleadores en eliminatorias
        tb.Label(frm_izq, text="⚽ Máximos Goleadores", font="-size 11 -weight bold", bootstyle="inverse-success").pack(fill=X, pady=(10, 5))
        self.tree_elim_goles = tb.Treeview(frm_izq, columns=("jugador", "goles"), show="headings", height=10, bootstyle="success")
        ...
        self.combo_fase_elim = tb.Combobox(frm_cruces, values=fases_para_tipo(self.tipo), state="readonly", width=22)
        ...
        tb.Button(frm_cruces, text="⚔️ Generar Cruce", bootstyle=SUCCESS, command=self._crear_cruce_eliminatoria).grid(row=1, column=4, columnspan=2, sticky="ew", padx=5, pady=4)

        # Cuadro del torneo
        tb.Label(frm_der, text="🏆 Cuadro del Torneo", font="-size 12 -weight bold", bootstyle="inverse-primary").pack(fill=X, pady=(0, 5))
        cols_cuadro = ("fase", "fecha", "partido", "resultado")
        self.tree_cuadro = tb.Treeview(frm_der, columns=cols_cuadro, show="headings", height=18, bootstyle="primary")
        ...
        self.tree_cuadro.tag_configure("final", background="#DC143C", foreground="white", font=("Segoe UI", 10, "bold"))
        self.tree_cuadro.tag_configure("campeon", background="#32CD32", foreground="black", font=("Segoe UI", 13, "bold"))
    ...
    def _ui_equipos_jugadores_copa(self, parent):
        ...
        t_eq = tb.Frame(nb2)
        nb2.add(t_eq, text="Equipos / Selecciones")
        t_jug = tb.Frame(nb2)
        nb2.add(t_jug, text="Jugadores")
    ...
    def _ui_exportar(self, parent):
        ...
        tb.Label(frm, text="Generador de Informes", font="-size 14 -weight bold").grid(row=0, column=0, columnspan=2, pady=(0, 20))
        tb.Label(frm, text="Contenido:").grid(row=1, column=0, sticky=W, padx=10, pady=8)
        self.combo_exp_tipo = tb.Combobox(frm, state="readonly", width=32, values=["Tabla de Posiciones", "Lista de Goleadores", "Resultados (partidos jugados)"])
        ...
        tb.Button(frm, text="💾 Generar y Guardar", bootstyle=SUCCESS, command=self._on_exportar, width=35).grid(row=3, column=0, columnspan=2, pady=25)
    ...
    def _refresh_equipos_copa(self):
        ...
        if not hasattr(self, "tree_clas"):
            return
        self.tree_clas.delete(*self.tree_clas.get_children())

        conn = _conn(self.db_path)
        equipos = conn.execute("SELECT id, nombre, ciudad FROM equipos ORDER BY nombre").fetchall()
        conn.close()

        for i, row in enumerate(equipos, 1):
            eq = dict(row)
            self.tree_clas.insert("", "end", values=(i, eq["nombre"], eq.get("ciudad", ""), "", "", "", "", "", "", "", ""))
    ...
    def _refresh_clasificacion(self):
        ...
        for i, _eq in enumerate(clas, 1):
            ...
            self.tree_clas.insert(
                "", "end", values=(i, eq.get("nombre", ""), eq.get("pj", 0), eq.get("pg", 0), eq.get("pe", 0), eq.get("pp", 0), eq.get("gf", 0), eq.get("gc", 0), f"+{dif}" if dif > 0 else str(dif), eq.get("pts", 0), racha, rend_local, rend_visitante)
            )

    def _refresh_jugadores(self):
        if not hasattr(self, "tree_jugadores"):
            return
        self.tree_jugadores.delete(*self.tree_jugadores.get_children())
        for _j in self.db.obtener_jugadores_completo():
            j = dict(_j)
            self.tree_jugadores.insert("", "end", values=(j["id"], j["nombre"], j.get("equipo_nombre", ""), j.get("goles_marcados", 0)))

    def _refresh_partidos(self):
        if not hasattr(self, "tree_partidos"):
            return
    ...
    def _refresh_goleadores(self):
        if not hasattr(self, "tree_goleadores"):
            return
        self.tree_goleadores.delete(*self.tree_goleadores.get_children())
        for i, g in enumerate(self.db.obtener_goleadores_top(), 1):
            self.tree_goleadores.insert("", "end", values=(i, g["jugador"], g["equipo"], g["goles"]))

    def _obtener_racha(self, nombre_equipo):
        """Obtiene racha de últimos 5 partidos: V=Victoria, D=Derrota, E=Empate."""
        try:
            conn = _conn(self.db_path)
            eq_id = conn.execute("SELECT id FROM equipos WHERE nombre=?", (nombre_equipo,)).fetchone()
            ...
            racha = ""
            for p in reversed(partidos):  # Ordenar cronológicamente
                es_local = p[2] == eq_id
                gl, gv = p[0], p[1]
            ...
            return racha if racha else "N/A"
        except:
            return "N/A"

    def _obtener_rendimiento_local(self, nombre_equipo):
        """Calcula rendimiento local: puntos en casa / puntos posibles en casa (%)."""
        try:
            conn = _conn(self.db_path)
            eq_id = conn.execute("SELECT id FROM equipos WHERE nombre=?", (nombre_equipo,)).fetchone()
            ...
            puntos_local = 0
            for gl, gv in partidos_local:
                if gl > gv:
                    puntos_local += 3
                elif gl == gv:
                    puntos_local += 1

            puntos_posibles = len(partidos_local) * 3
            rendimiento = int((puntos_local / puntos_posibles * 100)) if puntos_posibles > 0 else 0

            return f"{rendimiento}%"
        except:
            return "0%"

    def _obtener_rendimiento_visitante(self, nombre_equipo):
        """Calcula rendimiento visitante: puntos fuera / puntos posibles fuera (%)."""
        try:
            conn = _conn(self.db_path)
            eq_id = conn.execute("SELECT id FROM equipos WHERE nombre=?", (nombre_equipo,)).fetchone()
            ...
            partidos_visitante = conn.execute(
                """
                SELECT goles_local, goles_visitante
                FROM partidos
            ...
            puntos_visitante = 0
            for gl, gv in partidos_visitante:
                if gv > gl:
                    puntos_visitante += 3
                elif gv == gl:
                    puntos_visitante += 1

            puntos_posibles = len(partidos_visitante) * 3
            rendimiento = int((puntos_visitante / puntos_posibles * 100)) if puntos_posibles > 0 else 0
    ...
    def _on_limpiar_duplicados_jugadores(self):
        ...
        if not messagebox.askyesno("Confirmar", "¿Limpiar duplicados?\nSe mantendrá un jugador de cada (nombre, equipo) única.\n" "¡Esto es irreversible!"):
            return
        try:
            conn = _conn(self.db_path)
    ...
    def _on_borrar_todos_jugadores(self):
        ...
        if not messagebox.askyesno("⚠️ ATENCIÓN", "¿BORRAR TODOS los jugadores de este torneo?\n" "¡Esta acción es IRREVERSIBLE!\n\n" "Haz clic en OK solo si estás seguro."):
            return
        if not messagebox.askyesno("Confirmación Final", "¿ESTÁS SEGURO?\nSe borrarán todos los jugadores permanentemente."):
            return
    ...
    def _on_cargar_jornada_csv(self):
        if messagebox.askyesno("Limpiar pendientes", "¿Eliminar partidos pendientes antes de importar?"):
    ...
    def _on_registrar_resultado(self):
        ...
        self.entry_gl.delete(0, "end")
        self.entry_gv.delete(0, "end")
        self._refresh_all()
    ...
    def _on_partido_seleccionado_ver_goleadores(self, event=None):
        ...
        if hasattr(self, "combo_gol_jugador"):
            ...
            if jugadores:
                self.combo_gol_jugador.set(jugadores[0])
    ...
    def _on_aniadir_goleador_partido(self):
        item = self.tree_partidos.selection() if hasattr(self, "tree_partidos") else None
        if not item:
            messagebox.showwarning("Aviso", "Selecciona primero un partido en la tabla.")
            return
        vals = self.tree_partidos.item(item, "values")
        pid = int(vals[0])
        res = vals[4]  # "2–0" o "–"
        if res == "–":
            messagebox.showwarning("Aviso", "Registra el resultado del partido antes de añadir goleadores.")
            return
        ...
        if not jid:
            messagebox.showerror("Error", "Jugador no encontrado en la BD.")
            return

        conn = _conn(self.db_path)
        p = conn.execute("SELECT equipo_local_id, equipo_visitante_id FROM partidos WHERE id=?", (pid,)).fetchone()
        if not p:
            conn.close()
            return
        ...
        if ya + goles > permitidos:
            conn.close()
            messagebox.showerror("Límite superado", f"El equipo marcó {permitidos} gol(es). Ya asignados: {ya}. " f"Solo puedes añadir {max(0, permitidos-ya)} más.")
            return

        conn.execute("INSERT INTO goleadores_partido (partido_id,jugador_id,goles) VALUES (?,?,?)", (pid, jid, goles))
    ...
    def _on_limpiar_goleadores_partido(self):
        ...
        if not item:
            return
        pid = int(self.tree_partidos.item(item, "values")[0])
        if not messagebox.askyesno("Confirmar", "¿Borrar todos los goleadores de este partido?"):
            return
        # Restar los goles a los jugadores antes de borrar
        conn = _conn(self.db_path)
        filas = conn.execute("SELECT jugador_id, goles FROM goleadores_partido WHERE partido_id=?", (pid,)).fetchall()
        for f in filas:
            conn.execute("UPDATE jugadores SET goles_marcados=MAX(0,goles_marcados-?) WHERE id=?", (f["goles"], f["jugador_id"]))
        conn.execute("DELETE FROM goleadores_partido WHERE partido_id=?", (pid,))
        conn.commit()
        conn.close()
        self._on_partido_seleccionado_ver_goleadores()
    ...
    def _on_generar_acta_pdf(self):
        """Genera acta en PDF del partido seleccionado."""
        item = self.tree_partidos.selection() if hasattr(self, "tree_partidos") else None
        if not item:
        ...
        pid = int(self.tree_partidos.item(item, "values")[0])
        self._generar_acta_pdf(pid)

    def _on_generar_acta_pdf_mundial(self):
        """Genera acta en PDF del partido seleccionado (Mundial/Euro)."""
        ...
        try:
            ...
            messagebox.showerror("Error", "No se pudo extraer el ID del partido")

    def _on_generar_acta_champions(self):
        """Genera acta en PDF del partido seleccionado (Champions)."""
        txt = self.combo_partido_champions.get() if hasattr(self, "combo_partido_champions") else ""
        if not txt:
            messagebox.showwarning("Aviso", "Selecciona un partido primero")
            return
    ...
    def _on_jornada_filtrada(self, event=None):
        ...
        if jornada == "-- Archivo Histórico --":
            ...
            jornada = None
        partidos = self.db.obtener_partidos_con_equipos(fecha_filtro=jornada, mostrar_archivados=arch)
        ...
        for p in partidos:
            ...
            self.tree_partidos.insert("", "end", values=(p["id"], p["fecha"], p["nombre_equipo_local"], p["nombre_equipo_visitante"], res, estado))
    ...
    def _on_guardar_clasificacion_manual(self):
        ...
        try:
            ...
            messagebox.showerror("Error", "No se pudieron leer las estadísticas.")
            return
        nv = {}
        for k, v in zip(("PJ", "PG", "PE", "PP", "GF", "GC", "PTS"), (pj, pg, pe, pp, gf, gc, pts)):
            ...
            if r is None:
                return
            nv[k] = r
        self.db.actualizar_clasificacion_manual(nombre, nv["PJ"], nv["PG"], nv["PE"], nv["PP"], nv["GF"], nv["GC"], nv["PTS"])
        self._refresh_clasificacion()
    ...
    def _on_editar_goles_totales_manual(self, event):
        ...
        nombre = vals[1]
        actual = int(vals[3])
        nuevo = simpledialog.askinteger("Goles totales", f"Nuevo total de goles para {nombre}:", initialvalue=actual)
    ...
    def _on_nueva_temporada(self):
        if not messagebox.askyesno("Nueva Temporada", "Esto borrará TODOS los resultados y estadísticas.\n¿Continuar?"):
            return
        conn = _conn(self.db_path)
    ...
    def _on_simulacro_historial(self):
        conn = _conn(self.db_path)
        if "historial_simulacros" not in {r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()}:
            conn.execute("CREATE TABLE historial_simulacros (id INTEGER PRIMARY KEY, fecha TEXT, json TEXT)")
            conn.commit()
        snap = {"fecha": datetime.now().isoformat(), "clasificacion": [dict(r) for r in conn.execute("SELECT * FROM equipos").fetchall()]}
        conn.execute("INSERT INTO historial_simulacros (fecha,json) VALUES (?,?)", (snap["fecha"], json.dumps(snap, ensure_ascii=False)))
        conn.commit()
        ...
        tree_h.column("id", width=60, anchor="center")
        tree_h.heading("fecha", text="Fecha y Hora")
        tree_h.column("fecha", width=340)
    ...
    def _cargar_partidos_csv(self, archivo_csv):
        """Carga partidos desde CSV (Champions, Eurocopa, etc.)"""
        import csv
        ...
        try:
            with open(archivo_csv, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    partidos.append({"local": row["equipo_local"].strip(), "visitante": row["equipo_visitante"].strip(), "fecha": row["fecha"], "fase": row["fase"], "jornada": row["jornada"]})
        except Exception as e:
    ...
    def _cargar_grupo_seleccionado(self):
        ...
        if not hasattr(self, "grupo_seleccionado"):
            return
        ...
        try:
            ...
            for p in partidos:
                ...
                res = f"{gl}–{gv}" if gl != -1 else "–"
                if hasattr(self, "tree_partidos_grupo"):
                    self.tree_partidos_grupo.insert("", "end", values=(p["fecha"], p["local"], p["visitante"], res, p["estadio"] or ""))
                ...
                if hasattr(self, "tree_posiciones_grupo"):
                    ...
                    if self.tipo == "mundial":
                        if pos <= 2:
                            ...
                            tag = ("eliminado",)
                    self.tree_posiciones_grupo.insert("", "end", values=(pos, r["nombre"], r["pj"], r["pg"], r["pe"], r["pp"], r["gf"], r["gc"], r["pts"]), tags=tag)
    ...
    def _cargar_grupos(self):
        if not hasattr(self, "tree_grupos"):
            return
        self._recalcular_grupos()
        ...
        conn = _conn(self.db_path)
        tiene_grupos = conn.execute("SELECT COUNT(*) FROM equipos WHERE grupo IS NOT NULL AND grupo != ''").fetchone()[0] > 0
        if tiene_grupos:
            rows = conn.execute("""
        ...
        for pos, r in enumerate(rows, 1):
            if self.tipo == "copa":
                ...
                tag = ()
            self.tree_grupos.insert("", "end", values=(r["grupo"], r["nombre"], r["pj"], r["pg"], r["pe"], r["pp"], r["gf"], r["gc"], r["pts"]), tags=tag)
        self._refresh_goleadores_mundial()
    ...
    def _desempatar_mundial_h2h(self, grupo_eq, grupo_id, conn):
        ...
        for pts in sorted(pts_map.keys(), reverse=True):
            ...
            if len(eqs) == 1:
                ...
                eqs_sort = sorted(
                    eqs, key=lambda e: (-(grupo_eq[e]["gf"] - grupo_eq[e]["gc"]), -grupo_eq[e]["gf"], -calc_fair_play(e), grupo_eq[e].get("ranking_fifa", 999))  # DG (desc)  # GF (desc)  # Fair Play (menos tarjetas = mejor)  # FIFA Ranking (asc)
                )
    ...
    def _recalcular_grupos(self):
        ...
        try:
            ...
            # Obtener partidos
            try:
                partidos = conn.execute(f"SELECT equipo_local_id, equipo_visitante_id, goles_local, goles_visitante, grupo FROM partidos WHERE {where}").fetchall()
            except:
                # Si grupo no existe aún, hacer sin ella
                partidos = conn.execute(f"SELECT equipo_local_id, equipo_visitante_id, goles_local, goles_visitante FROM partidos WHERE {where}").fetchall()
            ...
            if self.tipo == "mundial":
                try:
                    grupos = conn.execute("SELECT DISTINCT grupo FROM partidos WHERE grupo IS NOT NULL").fetchall()
                    for gid in grupos:
                        grupo_id = gid["grupo"] if isinstance(gid, dict) else gid[0]
                        eq_grupo = {}
                        for r in conn.execute("SELECT id, nombre, pj, pg, pe, pp, gf, gc, pts, ranking_fifa FROM equipos WHERE grupo=?", (grupo_id,)):
                            eq_grupo[r["id"]] = dict(r) if hasattr(r, "keys") else {"id": r[0], "nombre": r[1], "pj": r[2], "pg": r[3], "pe": r[4], "pp": r[5], "gf": r[6], "gc": r[7], "pts": r[8], "ranking_fifa": r[9]}
                        if eq_grupo:
    ...
    def _refresh_goleadores_mundial(self):
        if not hasattr(self, "tree_gol_mundial"):
            return
    ...
    def _cargar_partidos_mundiales(self):
        if not hasattr(self, "tree_mundial_partidos"):
            return
        ...
        if hasattr(self, "combo_filtro_fase_cal"):
            fases_db = [r[0] for r in conn.execute("SELECT DISTINCT fase FROM partidos WHERE fase IS NOT NULL ORDER BY fase").fetchall()]
        ...
        for r in rows:
            ...
            self.tree_mundial_partidos.insert("", "end", values=(r["id"], r["fecha"], partido_txt, res, r["estadio"], r["fase"]))
    ...
    def _on_seleccion_partido_mundial(self, event=None):
        ...
        try:
            ...
            if self.tipo == "mundial" and hasattr(self, "spin_am_l"):
                self.spin_am_l.delete(0, "end")
                self.spin_am_l.set(p["tarjetas_amarillas_local"] or 0)
                self.spin_am_v.delete(0, "end")
                self.spin_am_v.set(p["tarjetas_amarillas_visitante"] or 0)
                self.spin_ro_l.delete(0, "end")
                self.spin_ro_l.set(p["tarjetas_rojas_local"] or 0)
                self.spin_ro_v.delete(0, "end")
                self.spin_ro_v.set(p["tarjetas_rojas_visitante"] or 0)
            ...
            equipos = []
            if n_loc:
                equipos.append(n_loc["nombre"])
            if n_vis:
                equipos.append(n_vis["nombre"])
            self.combo_eq_mundial["values"] = equipos
            if equipos:
                self.combo_eq_mundial.set(equipos[0])
    ...
    def _on_eq_mundial_cambiado(self, event=None):
        ...
        if jugadores:
            self.combo_jug_mundial.set(jugadores[0])
    ...
    def _grabar_resultado_mundial(self):
        ...
        try:
            ...
            messagebox.showerror("Error", "Introduce números válidos (≥ 0).")
            return
        ...
        if a_loc > gl or a_vis > gv:
            conn.close()
            messagebox.showerror("Cuadre", f"Ya hay {a_loc} goles locales y {a_vis} visitantes registrados.\n" "Edita los goleadores antes de bajar el marcador.")
            return
        conn.execute("UPDATE partidos SET goles_local=?,goles_visitante=?,jugado=1 WHERE id=?", (gl, gv, pid))
        ...
        if self.tipo == "mundial" and hasattr(self, "spin_am_l"):
            try:
                ...
                conn.execute("UPDATE partidos SET tarjetas_amarillas_local=?,tarjetas_amarillas_visitante=?," "tarjetas_rojas_local=?,tarjetas_rojas_visitante=? WHERE id=?", (am_l, am_v, ro_l, ro_v, pid))
        ...
        if self.tipo == "copa" and hasattr(self, "entry_pen_local"):
            try:
                ...
                conn.execute("UPDATE partidos SET penaltis_local=?,penaltis_visitante=? WHERE id=?", (pl, pv, pid))
    ...
    def _abrir_editar_resultado_mundial(self):
        """Abre ventana para editar resultado de un partido en Mundial."""
        txt = self.combo_partido_mundial.get()
        if not txt:
            messagebox.showwarning("Aviso", "Selecciona un partido primero.")
            return
        ...
        tb.Label(frm, text=f"{p['loc_nom']}  vs  {p['vis_nom']}", font="-size 13 -weight bold").pack(pady=(0, 15))

        tb.Label(frm, text="Goles Local:").pack(anchor=W, pady=5)
        e_gl = tb.Entry(frm, width=8)
        e_gl.pack(anchor=W, pady=(0, 10))
        if p["goles_local"] != -1:
            e_gl.insert(0, str(p["goles_local"]))

        tb.Label(frm, text="Goles Visitante:").pack(anchor=W, pady=5)
        e_gv = tb.Entry(frm, width=8)
        e_gv.pack(anchor=W, pady=(0, 15))
        if p["goles_visitante"] != -1:
            e_gv.insert(0, str(p["goles_visitante"]))
        ...
        def _guardar():
            ...
            c2.execute("UPDATE partidos SET goles_local=?,goles_visitante=?,jugado=1 WHERE id=?", (gl, gv, pid))
            c2.commit()
            c2.close()
            self._cargar_grupo_seleccionado()
            win.destroy()

        tb.Button(frm, text="💾 Guardar", bootstyle=SUCCESS, command=_guardar).pack(fill=X, pady=(10, 5))
        tb.Button(frm, text="❌ Cancelar", bootstyle="danger-outline", command=win.destroy).pack(fill=X)
    ...
    def _grabar_gol_mundial(self):
        ...
        try:
            ...
            if not p:
                ...
                return
            if p["goles_local"] == -1:
                messagebox.showerror("Bloqueo", "Graba el marcador antes de asignar goleadores.")
                return
            ...
            if ya + goles > permitidos:
                messagebox.showerror("Límite", f"{equipo} marcó {permitidos} goles total.\n" f"Ya asignados: {ya}. Solo puedes añadir {permitidos-ya} más.")
                return
            jid = self.db.obtener_id_jugador(jugador)
            if not jid:
    ...
    def _cargar_eliminatorias(self):
        if not hasattr(self, "tree_clasificados"):
            return
        ...
        for r in rows:
            ...
            if pos <= 2:
                ...
                terceros.append({"grupo": grp, "pais": r["nombre"], "pts": r["pts"], "dif": r["gf"] - r["gc"], "gf": r["gf"]})
        if self.tipo == "mundial":
        ...
        for r in sorted(elim_rows, key=lambda x: fase_orden.get(x["fase"], 99)):
            ...
            fase = r["fase"]
            tag = ""
            if "Playoffs" in fase or "Dieciseis" in fase:
                tag = "r16"
            elif "Octavos" in fase:
                tag = "octavos"
            elif "Cuartos" in fase:
                tag = "cuartos"
            elif "Semi" in fase:
                tag = "semis"
            elif "Final" in fase:
                tag = "final"
            self.tree_cuadro.insert("", "end", values=(fase, r["fecha"], f"{r['loc']} vs {r['vis']}", res), tags=(tag,))
            if fase == "Final" and r["gl"] is not None and r["gl"] != -1:
                campeon = r["loc"] if r["gl"] > r["gv"] else (r["vis"] if r["gv"] > r["gl"] else f"{r['loc']} o {r['vis']} (pen.)")
        if campeon:
            self.tree_cuadro.insert("", "end", values=("🏆 CAMPEÓN", "⭐⭐⭐", f"¡{campeon.upper()}!", "⭐⭐⭐"), tags=("campeon",))
        # Goleadores en eliminatorias
    ...
    def _crear_cruce_eliminatoria(self):
        ...
        if not all([fase, local, visit, fecha]):
            messagebox.showwarning("Aviso", "Rellena fase, equipos y fecha.")
            return
        if local == visit:
            messagebox.showwarning("Aviso", "El local y visitante no pueden ser el mismo.")
            return
        ...
        self._cargar_eliminatorias()
        self._cargar_partidos_mundiales()
        messagebox.showinfo("Cruce creado", f"{fase}: {local} vs {visit} → partido programado.\n" "Puedes grabar el resultado desde la pestaña de Grupos.")
    ...
    def _abrir_editar_partido(self):
        ...
        tb.Label(frm, text=f"{p['loc_nom']}  vs  {p['vis_nom']}", font="-size 13 -weight bold").grid(row=0, column=0, columnspan=4, pady=(0, 15))
        tb.Label(frm, text="Goles Local:").grid(row=1, column=0, sticky=W, pady=5)
        e_gl = tb.Entry(frm, width=8)
        e_gl.grid(row=1, column=1, padx=5)
        tb.Label(frm, text="Goles Visitante:").grid(row=1, column=2, sticky=W, pady=5)
        e_gv = tb.Entry(frm, width=8)
        e_gv.grid(row=1, column=3, padx=5)
        gl_act = p["goles_local"]
        gv_act = p["goles_visitante"]
        if gl_act != -1:
            e_gl.insert(0, str(gl_act))
        if gv_act != -1:
            e_gv.insert(0, str(gv_act))

        tb.Label(frm, text="Fecha:").grid(row=2, column=0, sticky=W, pady=5)
        e_fecha = tb.Entry(frm, width=15)
        e_fecha.grid(row=2, column=1, padx=5)
        e_fecha.insert(0, p["fecha"] or "")

        # Goleadores actuales
        tb.Label(frm, text="Goleadores registrados:", font="-weight bold").grid(row=3, column=0, columnspan=4, pady=(15, 5), sticky=W)
        tree_gp = tb.Treeview(frm, columns=("jug", "goles"), show="headings", height=5)
        tree_gp.heading("jug", text="Jugador")
        tree_gp.heading("goles", text="Goles")
        ...
        tb.Button(frm, text="🗑 Limpiar goleadores", bootstyle="danger-outline", command=_limpiar_goles).grid(row=5, column=0, columnspan=2, sticky="ew", pady=8)
        ...
        def _guardar():
            try:
                ...
                return
            c2 = _conn(self.db_path)
            c2.execute("UPDATE partidos SET goles_local=?,goles_visitante=?,jugado=1,fecha=? WHERE id=?", (gl, gv, e_fecha.get().strip(), pid))
            c2.commit()
            c2.close()
            self._refresh_all()
            win.destroy()

        tb.Button(frm, text="💾 Guardar Cambios", bootstyle=SUCCESS, command=_guardar).grid(row=5, column=2, columnspan=2, sticky="ew", pady=8)

    # ──────────────────────────────────────────────────────
    # GENERADOR DE ACTAS EN PDF
    # ──────────────────────────────────────────────────────

    def _generar_acta_pdf(self, partido_id):
        """Genera un acta oficial en PDF de un partido."""
        try:
            from reportlab.lib.pagesizes import letter
        ...
        try:
            ...
            goleadores_visitante = conn.execute(
                """
                SELECT j.nombre, COUNT(*) as goles
                FROM goleadores_partido gp
                JOIN jugadores j ON gp.jugador_id = j.id
            ...
            torneo_info = leer_torneo_info(self.db_path)
            conn.close()

            # Crear carpeta de actas si no existe
            carpeta_actas = os.path.join(_carpeta_bbdd(), "Actas_PDF")
            os.makedirs(carpeta_actas, exist_ok=True)

            # Nombre del archivo
            fecha_limpia = partido["fecha"].replace("-", "")
            nombre_archivo = f"ACTA_{fecha_limpia}_{partido['local'].replace(' ', '_')}_vs_{partido['visitante'].replace(' ', '_')}.pdf"
            ruta_pdf = os.path.join(carpeta_actas, nombre_archivo)

            # Crear PDF
            doc = SimpleDocTemplate(ruta_pdf, pagesize=letter, rightMargin=0.5 * inch, leftMargin=0.5 * inch, topMargin=0.5 * inch, bottomMargin=0.5 * inch)

            story = []
            styles = getSampleStyleSheet()

            # Estilo personalizado para encabezado
            style_titulo = ParagraphStyle("Titulo", parent=styles["Heading1"], fontSize=24, textColor=colors.HexColor("#003366"), spaceAfter=6, alignment=TA_CENTER, fontName="Helvetica-Bold")

            style_subtitulo = ParagraphStyle("Subtitulo", parent=styles["Normal"], fontSize=12, textColor=colors.HexColor("#666666"), spaceAfter=12, alignment=TA_CENTER)

            # Encabezado
            story.append(Paragraph(f"⚽ {torneo_info.get('nombre', 'TORNEO')}", style_titulo))
            story.append(Paragraph(f"ACTA OFICIAL DE PARTIDO", style_subtitulo))
            story.append(Spacer(1, 0.2 * inch))

            # Info del partido
            info_style = ParagraphStyle("Info", parent=styles["Normal"], fontSize=11, alignment=TA_CENTER, spaceAfter=6)
            story.append(Paragraph(f"📅 Fecha: {partido['fecha']}", info_style))
            if partido["estadio"]:
                story.append(Paragraph(f"🏟️ Estadio: {partido['estadio']}", info_style))
            story.append(Spacer(1, 0.15 * inch))

            # Marcador
            marcador_style = ParagraphStyle("Marcador", parent=styles["Heading2"], fontSize=28, textColor=colors.HexColor("#003366"), spaceAfter=12, alignment=TA_CENTER, fontName="Helvetica-Bold")
            marcador = f"{partido['local']} <b>{partido['goles_local']}</b> - <b>{partido['goles_visitante']}</b> {partido['visitante']}"
            story.append(Paragraph(marcador, marcador_style))
            story.append(Spacer(1, 0.2 * inch))

            # Tabla de goleadores
            data_goleadores = [[f"<b>{partido['local']}</b>", "Goles", f"<b>{partido['visitante']}</b>", "Goles"]]

            max_goles = max(len(goleadores_local), len(goleadores_visitante))
            for i in range(max_goles):
                fila = []
                if i < len(goleadores_local):
                    fila.append(goleadores_local[i]["nombre"])
                    fila.append(str(goleadores_local[i]["goles"]))
                else:
                    fila.append("")
                    fila.append("")

                if i < len(goleadores_visitante):
                    fila.append(goleadores_visitante[i]["nombre"])
                    fila.append(str(goleadores_visitante[i]["goles"]))
                else:
                    fila.append("")
                    fila.append("")

                data_goleadores.append(fila)

            # Si no hay goleadores
            if max_goles == 0:
                data_goleadores.append(["Sin goles", "", "Sin goles", ""])

            tabla = Table(data_goleadores, colWidths=[2.5 * inch, 0.5 * inch, 2.5 * inch, 0.5 * inch])
            tabla.setStyle(
                TableStyle(
                    [
            ...
            story.append(Paragraph(f"Documento generado automáticamente | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", footer_style))

            # Generar PDF
            doc.build(story)

            messagebox.showinfo("Éxito", f"Acta generada:\n{ruta_pdf}")
    ...
    def _on_exportar(self):
        ...
        ruta = filedialog.asksaveasfilename(defaultextension=ext, filetypes=[(fmt, f"*{ext}")], initialfile=f"informe_{datetime.now().strftime('%Y%m%d_%H%M')}{ext}")
        if not ruta:
            return
        try:
            if tipo_exp == "Tabla de Posiciones":
                ...
                filas = [(i + 1, d["nombre"], d.get("ciudad", "") if isinstance(d, dict) else (d["ciudad"] or ""), d["pj"], d["pg"], d["pe"], d["pp"], d["gf"], d["gc"], d["gf"] - d["gc"], d["pts"]) for i, d in enumerate(datos)]
                ...
                filas = [(d["id"], d["fecha"], d["nombre_equipo_local"], d["nombre_equipo_visitante"], f"{d['goles_local']}–{d['goles_visitante']}" if d["jugado"] else "–") for d in datos]
            ...
            if fmt == "CSV":
                ...
                with open(ruta, "w", encoding="utf-8") as f:
                    json.dump([dict(zip(cabecera, r)) for r in filas], f, ensure_ascii=False, indent=2)
    ...
    def _on_generar_liga_completa(self):
        ...
        if len(equipos) != 36:
            messagebox.showerror("Error", f"Se necesitan exactamente 36 equipos para la Liga.\nTienes {len(equipos)}.")
            return
        if not messagebox.askyesno("Confirmar", "¿Borrar partidos de liga previos y generar 8 jornadas (144 partidos)?"):
            return
        conn = _conn(self.db_path)
        ...
        for jor in range(1, 9):
            ...
            for i in range(18):
                ...
                conn.execute("INSERT INTO partidos (equipo_local_id,equipo_visitante_id,fase,jornada,fecha,goles_local,goles_visitante) VALUES (?,?,?,?,?,?,?)", (id_l, id_v, "Liga Regular", jornada_lbl, "2026", -1, -1))
    ...
    def _on_generar_octavos_automatico(self):
        ...
        if len(clasificacion) < 24:
            messagebox.showerror("Error", "Necesitas al menos 24 equipos con estadísticas.")
            return
        ganadores_playoff = self._obtener_ganadores_de_fase("Playoffs")
        if len(ganadores_playoff) < 8:
            messagebox.showwarning("Atención", f"Faltan resultados de Playoffs. Solo hay {len(ganadores_playoff)} ganadores.")
            return
    ...
    def _on_sorteo_manual(self):
        ...
        frm_cruces = tb.Frame(win)
        frm_cruces.pack(fill=BOTH, expand=True, pady=10)
        self._combos_sorteo = []

        def _actualizar(*_):
            for w in frm_cruces.winfo_children():
                w.destroy()
            self._combos_sorteo.clear()
            fase = fase_var.get()
            mapa = {"Playoffs": 8, "Octavos de final": 8, "Cuartos de final": 4, "Semifinal": 2, "Final": 1}
            n = mapa.get(fase, 4)
            fase_previa = {"Playoffs": "", "Octavos de final": "Playoffs", "Cuartos de final": "Octavos de final", "Semifinal": "Cuartos de final", "Final": "Semifinal"}.get(fase, "")
            if fase_previa:
        ...
        def _guardar():
            ...
            for cb1, cb2 in self._combos_sorteo:
                ...
                if not id1 or not id2:
                    continue
                conn.execute("INSERT INTO partidos (equipo_local_id,equipo_visitante_id,fase,jornada,fecha,goles_local,goles_visitante) VALUES (?,?,?,?,?,-1,-1)", (id1["id"], id2["id"], fase, "Ida" if ida_vuelta else "Único", "DD/MM/AAAA"))
                n += 1
                if ida_vuelta:
                    conn.execute("INSERT INTO partidos (equipo_local_id,equipo_visitante_id,fase,jornada,fecha,goles_local,goles_visitante) VALUES (?,?,?,?,?,-1,-1)", (id2["id"], id1["id"], fase, "Vuelta", "DD/MM/AAAA"))
                    n += 1
            conn.commit()
            conn.close()
            if n:
    ...
    def _cambiar_torneo(self):
        ...
        if torneo_seleccionado[0]:
            ...
            if not torneo_info:
                ...
                conn.execute("PRAGMA journal_mode = WAL")
                conn.executescript(SCHEMA)
                conn.execute("INSERT OR IGNORE INTO torneo_info (nombre,tipo,temporada) VALUES (?,?,?)", (torneo["nombre"], torneo["tipo"], ""))
                conn.commit()
                conn.close()
                torneo_info = leer_torneo_info(db_path)
    ...
    def _sincronizar_con_api(self):
        """Sincroniza todos los datos de MAESTRO con API Render."""
        import requests
        import sqlite3

        API_URL = "https://gestor-torneos-premium-2.onrender.com"
        MAESTRO_PATH = r"D:\Manu\Documentos\FUTBOL\Gestor_Torneos_Premium\MAESTRO"

        torneos_config = {
            "mundial": f"{MAESTRO_PATH}\\MUNDIAL\\mundial_2026_v2.db",
            "champions": f"{MAESTRO_PATH}\\CHAMPIONS\\champions_gui_v2.db",
            "euro": f"{MAESTRO_PATH}\\EURO\\eurocopa_2024_v2.db",
            "liga": f"{MAESTRO_PATH}\\LIGA\\liga_futbol_v2.db",
        }

        total_equipos = 0
        total_partidos = 0

        for torneo, db_path in torneos_config.items():
            try:
                conn = sqlite3.connect(db_path)
                conn.row_factory = sqlite3.Row
                ...
                # Enviar equipos a API
                for eq in equipos:
                    try:
                        requests.post(f"{API_URL}/api/equipos", json={"nombre": eq["nombre"], "ciudad": eq.get("ciudad", "")}, timeout=5)
                        total_equipos += 1
                    except:
                        pass
                ...
                partidos_json = []
                for p in partidos:
                    partidos_json.append({"fecha": p["fecha"], "equipo_local_id": p["equipo_local_id"], "equipo_visitante_id": p["equipo_visitante_id"], "goles_local": p["goles_local"], "goles_visitante": p["goles_visitante"]})

                json_path = r"D:\Manu\Documentos\FUTBOL\Gestor_Torneos_Premium\MAESTRO\web\public\data\partidos_{}.json".format(torneo)
                with open(json_path, "w", encoding="utf-8") as f:
                    json.dump(partidos_json, f, ensure_ascii=False, indent=2)

                total_partidos += len(partidos_json)

                print(f"✓ {torneo.upper()}: {len(equipos)} equipos, {len(partidos)} partidos")

                conn.close()
            except Exception as e:
                print(f"✗ Error en {torneo}: {e}")

        print(f"✓ Sincronización: {total_equipos} equipos, {total_partidos} partidos")

    def _al_cerrar(self):
        """Al cerrar, pregunta si sincronizar con API."""
        respuesta = messagebox.askyesno("Sincronizar", "¿Sincronizar datos con API antes de cerrar?\n\n(Necesario para ver cambios en iPhone/Web)", icon=messagebox.QUESTION)

        if respuesta:
            print("⏳ Sincronizando...")
            try:
                self._sincronizar_con_api()
                messagebox.showinfo("Éxito", "✓ Datos sincronizados con API")
            except Exception as e:
...
class DashboardPrincipal(tk.Frame):
    ...
    def __init__(self, root, db_path, callback_torneo_seleccionado):
        ...
        tb.Label(frm_header, text="🏆 GESTOR PREMIUM DE TORNEOS 2026", font="-size 20 -weight bold", bootstyle="inverse-info").pack(pady=10)
        tb.Label(frm_header, text="Selecciona el torneo que deseas gestionar", font="-size 12", foreground="#555").pack()
        ...
        self.torneos = [
            {"nombre": "⚽ LIGA", "tipo": "liga", "archivo": "LIGA/liga_futbol_v2.db"},
            {"nombre": "🏅 CHAMPIONS LEAGUE", "tipo": "champions", "archivo": "CHAMPIONS/champions_gui_v2.db"},
            {"nombre": "🌍 MUNDIAL 2026", "tipo": "mundial", "archivo": "MUNDIAL/mundial_2026_v2.db"},
            {"nombre": "🇪🇺 EUROCOPA 2024", "tipo": "euro", "archivo": "EURO/eurocopa_2024_v2.db"},
        ]
        ...
        for i, torneo in enumerate(self.torneos):
            ...
            tb.Label(frm_torneo, text=estado, font="-size 10", foreground="#666", justify="left").pack(pady=(10, 10), padx=10)

            # Botón seleccionar
            tb.Button(frm_torneo, text=f"Abrir {torneo['nombre'].split()[1]}", bootstyle=SUCCESS, width=30, command=lambda t=torneo: self._seleccionar_torneo(t)).pack(padx=10, pady=10)

        # Botón crear nuevo torneo
        frm_nuevo = tb.LabelFrame(frm_main, text=" ➕ CREAR NUEVO TORNEO ")
        frm_nuevo.grid(row=2, column=0, columnspan=2, padx=15, pady=15, sticky="ew")

        tb.Label(frm_nuevo, text="Crea un torneo personalizado con tu propia BD", font="-size 10", foreground="#666").pack(pady=(10, 10), padx=10)
        tb.Button(frm_nuevo, text="🆕 Crear Torneo Personalizado", bootstyle="warning", width=30, command=self._crear_nuevo_torneo).pack(padx=10, pady=10)
        # Footer
        frm_footer = tb.Frame(self)
        frm_footer.pack(fill=X, padx=20, pady=20)
        tb.Label(frm_footer, text="v2.0 - 2026 | Sistema flexible para Liga, Champions, Mundial, Eurocopa", font="-size 9", foreground="#999").pack()

    def _crear_nuevo_torneo(self):
        """Abre diálogo para crear un nuevo torneo personalizado."""

        def on_crear(resultado):
            if resultado:
                nombre = resultado["nombre"]
                tipo_torneo = resultado["tipo"]

                # Generar nombre de archivo
                archivo = f"{tipo_torneo}_{nombre.replace(' ', '_')}_custom.db"
                carpeta = _carpeta_bbdd()
                db_path = os.path.join(str(carpeta), archivo)

                # Verificar si ya existe
                if os.path.exists(db_path):
                    messagebox.showwarning("Aviso", f"El torneo '{nombre}' ya existe")
                    return
                ...
                try:
                    ...
                    conn.execute("PRAGMA journal_mode = WAL")
                    conn.executescript(SCHEMA)
                    conn.execute("INSERT INTO torneo_info (nombre, tipo, temporada) VALUES (?, ?, ?)", (nombre, tipo_torneo, ""))
                    conn.commit()
                    conn.close()

                    messagebox.showinfo("Éxito", f"Torneo '{nombre}' creado exitosamente.\n\nAhora está listo para importar equipos.")

                    # Cargar el nuevo torneo
                    torneo_info = {"nombre": nombre, "tipo": tipo_torneo, "temporada": ""}
                    self.callback(torneo_info)

                except Exception as e:
                    messagebox.showerror("Error", f"Error al crear torneo: {e}")

        DialogoCrearTorneo(self._root, on_crear=on_crear)
    ...
    def _obtener_estado_torneo(self, torneo):
        ...
        try:
            ...
            try:
                num_equipos = conn.execute("SELECT COUNT(*) FROM equipos").fetchone()[0]
                num_partidos = conn.execute("SELECT COUNT(*) FROM partidos").fetchone()[0]
            finally:
                conn.close()
...
def abrir_lobby(root):
    ...
    sw = lobby.winfo_screenwidth()
    sh = lobby.winfo_screenheight()
    lobby.geometry(f"520x340+{(sw-520)//2}+{(sh-340)//2}")

    tb.Label(lobby, text="⚽ Gestor Premium de Torneos", font="-size 16 -weight bold").pack(pady=(30, 5))
    tb.Label(lobby, text="Selecciona o crea el torneo que quieres gestionar", font="-size 10", foreground="#666666").pack(pady=(0, 25))

    card = tb.Frame(lobby, padding=10)
    card.pack(fill=X, padx=50)

    def _cargar():
        ruta = filedialog.askopenfilename(parent=lobby, title="Abrir base de datos", initialdir=carpeta_dbs, filetypes=[("Base de Datos SQLite", "*.db")])
        if ruta:
            db_elegida[0] = ruta
            lobby.destroy()
    ...
    tb.Button(card, text="📂  Cargar Torneo Existente", bootstyle="primary", width=38, command=_cargar).pack(pady=6)
    tb.Button(card, text="✨  Crear Torneo Nuevo", bootstyle="success-outline", width=38, command=_crear).pack(pady=6)
    tb.Button(card, text="✖  Salir", bootstyle="secondary-outline", width=38, command=_salir).pack(pady=6)
